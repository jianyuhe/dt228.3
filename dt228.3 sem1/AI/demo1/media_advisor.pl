...
medium(videocassette) :- stimulus_situation(visual),
                    ...
                    feedback(no).
% rule 12
medium(lecture_tutorial) :- stimulus_situation(physical_object),
                    stimulus_response(hands_on),
                    feedback(yes).
% rule 13
...
go :- 
   getEnvironment,
    getJob,
    feedback,
    ( stimulus_situation(QQ),
      nl, write('Stimulus situation is '), write(QQ),nl
	  ,
	  stimulus_response(SS), nl, write('stimulus_response is '), write(SS), nl
	  ,
