...
select FIRST_NAME, LAST_NAME, FIRST_NAME || ' ' || LAST_NAME AS emp_name,
 JOB_ID,e.MANAGER_ID, DEPARTMENT_ID, CITY, COUNTRY_NAME
from EMPLOYEES e
join departments d USING(DEPARTMENT_ID)
JOIN LOCATIONS l USING(LOCATION_ID)
JOIN COUNTRIES c USING(COUNTRY_ID);

select FIRST_NAME, LAST_NAME,count(s.JOB_ID) as "number of job"
from EMPLOYEES 
join JOB_HISTORY s using(EMPLOYEE_ID)
