...
const int BsensorPin = A1; //brightness

const int HsensorPin = A2; //humidity

int const potPin = A0; //add potentiometer  but not test
void setup() {
  Serial.begin(9600);
...
for(int i = 0; i < 12 ; i++)
{
  ...
  int rotation = map(potVal, 0, 1023, 0, 179);
  ...
  if (calls <= maxCalls) {
    ...
    String RowDataValue = String(temperature)+","+String(brightness)+","+String(Hsensor)+"%" + " ," + String(rotation);
  ...
  Serial.println("\nWaiting 5 mins\n");
  delay(300000); // wait5mins between AppendRow calls
  Serial.println("\n 5 mins already start next!\n");
}
Serial.println("\n already 1 hour stop!\n");
} 
